...
export class BorshSchema<_S extends BSE> {
   ...
   */
  static Option<T extends BorshSchema<BSE>>(
    value: T
  ): BorshSchema<Option<BSEType<T>>> {
   ...
   */
  static Array<T extends BorshSchema<BSE>>(
    value: T,
    length: number
  ): BorshSchema<ArrayBSE<BSEType<T>>> {
   ...
   * const buffer = borshSerialize(schema, messages);
   */
  static Vec<T extends BorshSchema<BSE>>(
    value: T
  ): BorshSchema<Vec<BSEType<T>>> {
    return BorshSchema.fromSchema({
   ...
   * const buffer = borshSerialize(schema, messages);
   */
  static HashSet<T extends BorshSchema<BSE>>(
    value: T
  ): BorshSchema<HashSet<BSEType<T>>> {
   ...
   */
  static Struct<R extends Record<string, BorshSchema<BSE>>>(
    fields: R
  ): BorshSchema<StructOf<R>> {
    return BorshSchema.fromSchema({
      struct: toStructTypeStruct(fields),
    } as Struct<R>);
  }
   ...
   */
  static Enum<R extends Record<string, BorshSchema<BSE>>>(
    variants: R
  ): BorshSchema<EnumOf<R>> {
    return BorshSchema.fromSchema({
      enum: toEnumTypeEnum(variants),
    } as Enum<R>);
  }
}

function toStructTypeStruct<R extends Record<string, BorshSchema<BSE>>>(
  fields: R
): StructType["struct"] {
...
function toEnumTypeEnum<R extends Record<string, BorshSchema<BSE>>>(
  variants: R
): EnumType["enum"] {
...
export type ArrayBSE<T> = Brand<
  T,
  Readonly<{ array: { type: Schema; len: number } }>
>;
...
type BSEType<T extends BorshSchema<BSE>> =
  T extends BorshSchema<infer S> ? S : never;
...
export function borshSerialize<S extends BSE>(
  schema: BorshSchema<S>,
  data: TypeOf<S>
): Buffer {
  return Buffer.from(serialize(schema.toSchema(), data));
}

export function borshDeserialize<S extends BSE>(
  schema: BorshSchema<S>,
  buffer: Uint8Array
): TypeOf<S> {
